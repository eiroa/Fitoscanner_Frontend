...
var app = angular.module('tip_eiroa_mauro_server_frontend');
  app.controller('PlagueSpeciesCtrl', function ($http, $location, $scope, ngTableParams,
			$filter, $window, $route, $rootScope, growl, dialogs,globalService,$translate,
			restServices,$timeout) {
	  
	  function translate(){
		  $translate('INDEX_PLAGUE_SPECIES').then(function (text) {
				$scope.title = text;
			    });
		  
		  $translate('FORM_NEW_PLAGUE').then(function (text) {
	  ...
	  $scope.inicializarVista = function() {
			...
			restServices.invokeGetPlagues($http, {},
					$scope.findOk,
